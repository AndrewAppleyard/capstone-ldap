...
dn: cn=UAFS_ADMINS,cn=groups,cn=accounts,${DS_SUFFIX_NAME}
cn: UAFS_ADMINS
objectclass: groupOfNames
EOF

ldapadd -D "cn=Directory Manager" -w ${DS_DM_PASSWORD} -H ldap://localhost:3389 -x <<EOF
dn: cn=UAFS_ADVISORS,cn=groups,cn=accounts,${DS_SUFFIX_NAME}
cn: UAFS_ADVISORS
objectclass: groupOfNames
EOF

ldapadd -D "cn=Directory Manager" -w ${DS_DM_PASSWORD} -H ldap://localhost:3389 -x <<EOF
dn: cn=UAFS_STUDENTS,cn=groups,cn=accounts,${DS_SUFFIX_NAME}
cn: UAFS_STUDENTS
objectclass: groupOfNames
EOF
